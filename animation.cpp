...
void loadGLTextures(const std::string& path, const aiScene* scene)
{
    ...
    for (unsigned int m = 0; m < scene->mNumMaterials; ++m)
    {
        ...
        if (scene->mMaterials[m]->GetTexture(aiTextureType_DIFFUSE, 0, &fileName) == AI_SUCCESS)
        {
            ...
            size_t lastIndex = realFileName.rfind('/');
            int realLastIndex = -1;
            ...
            if (lastIndex != std::string::npos)
            {
                realLastIndex = std::max((int)lastIndex, realLastIndex);
            }
            ...
            if (ilLoadImage((ILstring)realFileName.c_str()))   //if success
            {
                ...
                cout << "Texture:" << realFileName << " successfully loaded." << endl;
            ...
            {
                cout << "Couldn't load Image: " << realFileName << endl;
            }
        }
    }  //loop for material
}
...
//    loadModel("data2/Ana/Ana.x");
//    loadModel("data2/SpaceShip/SpaceShip.3ds");
    loadModel("data2/Spider/Spider.obj");
//    loadModel("data2/Truck/Truck.3ds");
//    loadModel("data2/ArmyPilot/ArmyPilot.x");
//    loadModel("data2/Dwarf/dwarf.x");
//    loadModel("data2/Mannequin/mannequin.fbx");
//    loadModel("data2/Dance.bvh");
//    loadModel("data2/Test.bvh");
...
int main(int argc, char** argv)
{
    ...
    glutInit(&argc, argv);
    glutInitDisplayMode(GLUT_RGB | GLUT_DOUBLE | GLUT_DEPTH | GLUT_DOUBLE);
